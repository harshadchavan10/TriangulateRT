...
def loadfiles(name):
	print '<<< Left: Loading '+name+' dictionary >>>'	
	starttime = time.time()
	dictionary = {}
	with open(name, "rb") as f1:
		dictionary = pickle.load(f1)
	print 'Left: The dictionary loaded in '+str(time.time()-starttime)+' sec length = '+str(len(dictionary))+'\n'
	return dictionary
...
def calc_count(pst, tst, tpt, sleft, sright, pleft, pright, tleft, fstruct, sd, pd):
	...
	for row in rows:
		...
		for target in targets:
			...
			trglefts = list(set(tleft[row][target]))

			if(len(trglefts) > 0):
				for pivot in pivots:	
		
					sources = fstruct[row][target][pivot].keys()
					pivlefts = list(set(pleft[row][pivot]))
		...
		if((row % 500) == 0):
			print 'Left: Row '+str(row)+' processed at '+str(time.time()-starttime)+' secs'
	...
	print 'Left: Combinations extracted in ' + str(time.time()-starttime) + ' secs\n'

	allsrcl = list(set(allsrc))
	sptl = list(set(sptlist))
	data =  [x.split(u'~~~') for x in sptl]

	print 'Left: ' + str(len(data)) + ' distinct src-piv-tgt combinations, '+ str(len(allsrcl)) + ' distinct source phrases'

	print '\nLeft: Generated the list'
	...
	print '\nLeft: Intermediate dictionaries are created successfully'
	print 'Left: ' + str(len(spdict)) + ' : source phrases in spdict' 
	print 'Left: ' + str(len(stdict)) + ' : source phrases in stdict'
	print 'Left: ' + str(len(ptdict)) + ' : pivot phrases in spdict'
	...
	finalCount = spdict
	print 'Left: ' + str(len(finalCount)) + ' : source phrases in finalCount' 
	print 'Left: ' + str(tot) + ' : combinations in finalCount' 
	print '\nLeft: Generated finalCount structure in ' + str(time.time()-starttime) + ' secs'
	...
	print 'Left: finalCounts updated in ' + str(time.time()-starttime) + ' secs'

	print '\nLeft: Dumping dictionary containing counts in final format (source-phrase > pivot-phrase > target-phrase > [count-list])\n'
	with open('finalLeftCountDict', "wb") as f2:
		pickle.dump(finalCount, f2)

	########################## Writing to file ##################################

	file1 = codecs.open('finalLeftCount', "w", "utf-8")
...
calc_count(pst, tst, tpt, sleft, sright, pleft, pright, tleft, fstruct, sd, pd)

print 'Left: Counts calculated for the given corpuses in '+str((time.time()-start))+' sec'
